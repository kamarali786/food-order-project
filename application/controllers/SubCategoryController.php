...
class subCategoryController extends CI_Controller
{
    ...
    // fetch subcategory data for ajax
    public function subcategory_data()
    {
        $id = $this->input->post('cate_id');
        $q =  $this->db->where(['category_id' => $id, 'status' => 1, 'is_delete' => 0])->get('subcategories');
        if ($q->num_rows()) {
            $output = '';
            //return $q->result();
            foreach ($q->result() as $val) {
                $output .= '<option value = "' . $val->subCategory_id . '">' . $val->subCategory_name . '</option>';
            }
            echo $output;
        } else {
            return false;
